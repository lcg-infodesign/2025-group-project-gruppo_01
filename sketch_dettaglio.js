...
function parseTableTotals() {
  ...
  for (let r = 0; r < tableRows.length; r++) {
    ...
    if (isNationalTotalRow) {
      ...
      // Parse national totals for SI/NO votes (accept multiple possible column names)
      const votiSi = (row['VOTI_SI'] || row['SUM of VOTI_SI'] || row['VOTI SI'] || row['NUMVOTISI'] || row['SUM of NUMVOTISI'] || '').trim();
      const votiNo = (row['VOTI_NO'] || row['SUM of VOTI_NO'] || row['VOTI NO'] || row['NUMVOTINO'] || row['SUM of NUMVOTINO'] || '').trim();
    ...
    if (isRegionTotalRow || isLikelyRegionTotal) {
      ...
      // Try different possible column names for regional totals
      const votiSi = (row['VOTI_SI'] || row['SUM of VOTI_SI'] || row['VOTI SI'] || row['NUMVOTISI'] || row['SUM of NUMVOTISI'] || '').trim();
      const votiNo = (row['VOTI_NO'] || row['SUM of VOTI_NO'] || row['VOTI NO'] || row['NUMVOTINO'] || row['SUM of NUMVOTINO'] || '').trim();
      ...
      if (regione && provincia && !regione.toUpperCase().includes('TOTAL')) {
        ...
        // Aggregate votes (accept various column name variants)
        const votiSi = (row['VOTI_SI'] || row['SUM of VOTI_SI'] || row['VOTI SI'] || row['NUMVOTISI'] || row['SUM of NUMVOTISI'] || '').trim();
        const votiNo = (row['VOTI_NO'] || row['SUM of VOTI_NO'] || row['VOTI NO'] || row['NUMVOTINO'] || row['SUM of NUMVOTINO'] || '').trim();
        if (votiSi) {
  ...
  for (let r = 0; r < tableRows.length; r++) {
    ...
    const votiSiStr = (row['VOTI_SI'] || row['SUM of VOTI_SI'] || row['VOTI SI'] || row['NUMVOTISI'] || row['SUM of NUMVOTISI'] || '').trim();
    const votiNoStr = (row['VOTI_NO'] || row['SUM of VOTI_NO'] || row['VOTI NO'] || row['NUMVOTINO'] || row['SUM of NUMVOTINO'] || '').trim();
...
function drawGenderChart() {
  ...
  textSize(32);
  fill(cFemale);
  text(`${pctF.toFixed(1)}%`, donneX, centerY + 2);
...
function drawQuesitiWindow() {
  ...
  textSize(20); // Dimensione titolo principale
  textFont('STIX Two Text');
  ...
  text("QUESITI", mapAreaWidth / 2 - 190 + 12, 130);
...
function drawPresidentSlider(x, y, w, h) {
  ...
  function drawSinglePresident(pName, pRole, pImgFilename, px, py, align, pLink, type) {
    ...
    // RUOLO
    fill(titleColor); textSize(16); textStyle(NORMAL); noStroke();
    if (hasSecondLine) {
...
function mousePressed() {
  ...
  // Expand vertical bounds slightly to improve clickability for single-line quesiti
  if (mouseX >= windowLeft + bgPadding && mouseX < windowLeft + windowWidth - bgPadding &&
    mouseY >= (quesitiAreaTop - 12) && mouseY < (quesitiAreaBottom + 12)) {
    ...
    const minQuesitoHeight = 40;
    const quesitiHeights = [];
    textSize(18); // Allineato con drawQuesitiWindow
    textStyle(NORMAL);
    ...
    for (let i = 0; i < quesiti2025.length; i++) {
      const quesitoHeight = quesitiHeights[i];
      const y = currentY;
      // Increase click area for better clickability (especially for single-line quesiti)
      const clickPadding = 8;
      const quesitoClickTop = y - clickPadding;
      const quesitoClickBottom = y + quesitoHeight + clickPadding;
